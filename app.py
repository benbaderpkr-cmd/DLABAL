...
if sel != "---":
    ...
    with tab4:
        ...
        try:
            ...
            st.warning("⚠️ Onglet vide ou inaccessible. Il sera initialisé au premier enregistrement.")

        # FORMULAIRE (Ton style 2 colonnes)
        with st.form(key=f"form_tho_{sel}"):
            ...
            if submit:
                try:
                    ...
                    # Forçage des colonnes dans le bon ordre
                    ...
                    st.success("✨ Données enregistrées ! Vérifiez Google Sheets.")
                    st.balloons()
                    
                except Exception as e:
                    if "200" in str(e):
                        st.success("✨ Succès (Code 200) !")
                        st.balloons()
                    else:
                        st.error(f"Erreur : {e}")
else:
